...
interface RegisterFormProps {
  onSubmit: (data: { email: string; password: string; name: string; role: 'user' }) => Promise<void>;
}
...
export default function RegisterForm({ onSubmit }: RegisterFormProps) {
  ...
  return(
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full max-w-md mx-auto px-4 sm:px-0 font-sans">
      ...
      <div className="flex flex-col w-full">
        ...
        <div className="relative w-full flex items-center">
          ...
          <span onClick={togglePassword} id="togglePassword" className="absolute right-3 cursor-pointer flex items-center h-full">
            {showPassword ? (
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" className="text-white/70">
      ...
      {/* Name */}
      <div className="flex flex-col w-full">
        <label htmlFor="name" className="mb-2 text-left text-white text-sm font-thin tracking-wide">
          Name
        </label>
        <input
          type="text"
      ...
      <div className="w-full">
        <button
          type="submit"
          className="w-full py-2 rounded-md bg-gradient-to-r from-gray-700 to-blue-gray-800 text-white hover:from-gray-600 hover:to-blue-gray-700 transition-all shadow-md cursor-pointer font-semibold text-base "
        >
